			...
			$.get(url,function(data){
				if(data.length>0){
					...
					$.each(data,function(i){
						...
						html += "<td><a class='btn btn-mini btn-info' title='编辑' onclick='editmenu(\""+this.MENU_ID+"\")'><i class='icon-edit'></i></a>" +
	" <a class='btn btn-mini btn-danger' title='删除' onclick='delmenu(\""+this.MENU_ID+"\",false)'><i class='icon-trash'></i></a>    <a class='btn btn-mini btn-purple'  title='图标'  onclick='editTb(\""+this.MENU_ID+"\")' ><i class='icon-picture'></i></a></td>";
						html += "</tr>";
